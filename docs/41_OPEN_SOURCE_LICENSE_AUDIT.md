...
> Repository note: this is the retained pre-split audit of the shared
> Mac/Windows codebase. See [`PROVENANCE.md`](../PROVENANCE.md) and
> [`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md) for the standalone
> Windows publication boundary.
...
The pre-split `git shortlog -sne --all` showed commits only from the two
public Git author identities `ainem-m` and `ainem`.
